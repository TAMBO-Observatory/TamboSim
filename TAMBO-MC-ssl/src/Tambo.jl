...
function inject!(
    ...
    if abs(pdg) in (12, 14, 16)
        # Neutrino injection (existing path)
        ...
        @llama_showprogress "Injecting" for frame in sim.results
            ...
            istate, cstate, fstate, wp = inject_event(
                pdg, earth, as, pl, cross_section, detector_props;
                tr_seed=tr_seed
            )
    ...
    elseif abs(pdg) == 13
        # Atmospheric muon injection (new path)
        @llama_showprogress "Injecting muons" for frame in sim.results
            istate, cstate, fstate, wp = inject_muon_event(
                pdg, earth, as, pl, detector_props
            )
            frame["$(outprefix)_initial_state"] = istate
            if !isnan(fstate.energy)
                frame["$(outprefix)_final_state"] = fstate
        ...
        error("Unsupported particle PDG: $pdg")
    end
end
