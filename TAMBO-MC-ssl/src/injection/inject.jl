...
function inject_muon_event(
    ...
    # Inject at the outermost atmospheric rock entry surface: the last intersection
    # where the backward ray exits rock into air (dot(normal, revd) > 0).
    # This is exactly the first rock surface on the forward trajectory from atmosphere.
    # Upgoing events find no such surface and are naturally rejected.
    atm_entry_idx = findlast(
        ix -> !isa(ix, SphereIntersection) && dot(ix.normal.point, revd.point) > 0,
        intersections
    )
    isnothing(atm_entry_idx) && return create_null_result(pdg, INJECTION_ERROR_AIR_ONLY, d, cs, T)
    energy = rand(pl)
    pdg_type = ParticleType(pdg)
    origin = intersections[atm_entry_idx].point
