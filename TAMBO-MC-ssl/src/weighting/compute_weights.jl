...
"""
    p_mc_surface(wp::WeightParameters{T}) -> Quantity

Monte Carlo probability density for surface-injected particles (no cross-section / column-depth factor).
Used for atmospheric muons and cosmic ray protons that are sampled directly on the detector surface.
"""
