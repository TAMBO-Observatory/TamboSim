...
- **3. Correct logical-node placement on rock exit.** `nextIntersect` sets a particle's post-exit logical node to `rock.getParent()` (the topmost spanned layer) regardless of where the exit point actually is, and the tracker leaves the particle sitting on the rock surface. Neither is directly usable: the geometrically-containing layer can differ from `getParent()`, and a particle left on the surface is inside the `boundaryPadding`-degenerate band. `tambo_shower` registers `RockExitRelocator`, a `BoundaryCrossingProcess` that on every rock exit corrects *both* — see the dedicated Q+A **"When a particle leaves the terrain rock, how does `RockExitRelocator` place it?"** below.

In summary: the rock is registered as an overlap-exclusion on each atmosphere layer the terrain spans and owned by the topmost of them, so both the point→medium lookup and the tracking step resolve to the rock from whatever layer a particle occupies while crossing the terrain; and `RockExitRelocator` re-resolves the logical node and nudges the particle off the surface on every rock exit (its own Q+A below), so the post-exit step uses the geometrically-correct medium from an unambiguous starting point.
...
- `detail/framework/geometry/TriangularMesh.inl` — `contains()` (lines 63–95) is a 3-ray parity vote that excludes hits within `boundaryPadding` (1 µm, line 88); a sub-surface point inside the mesh resolves through this same primitive.
...
- `tambo_shower.cpp` (local, [src/corsika/tambo_shower/src/](../src/corsika/tambo_shower/src/)) — terrain mesh registered as a StandardRock `HomogeneousMedium` via `excludeOverlapWith` on each spanned atmosphere layer and owned by the topmost spanned layer (the `CRITICAL` comment at the rock-volume registration). `RockExitRelocator` / `RockInterfaceTripwire` are covered in the rock-exit Q+A below.

</details>

<details>
<summary><h3>When a particle leaves the terrain rock, how does <code>RockExitRelocator</code> place it?</h3></summary>

When a particle crosses out of the terrain rock, two things are left in a state the next step cannot use directly, and `tambo_shower`'s `RockExitRelocator` (a `BoundaryCrossingProcess` in the process sequence, firing on every boundary crossing whose `from` node is the rock) fixes both:

- **The logical node.** `nextIntersect` sets the post-exit node to `rock.getParent()` — the topmost atmosphere layer the terrain spans (the rock is owned by it precisely so `getParent()` is a real layer, never the universe, which would erase the particle). That is not necessarily the layer that geometrically contains the exit point.
- **The position.** The tracker leaves the particle sitting *exactly on the rock surface*, inside the 1 µm `boundaryPadding` band where mesh geometry is degenerate.

**Resolving the layer — by sphere descent, deliberately not `getContainingNode`.** The relocator picks the layer *before* nudging, with the particle still on the surface. At that on-surface point `getContainingNode` is the wrong tool: it consults the rock through `excludes()`, and `TriangularMesh::contains()` at an on-surface point is degenerate — it drops the local face via the 1 µm `boundaryPadding` and votes by parity of the *remaining* crossings, which for non-convex terrain can come out "inside" — so it can resolve back to the very rock the particle is leaving and ping-pong the boundary with near-zero progress. The question actually needed is "which atmosphere *layer* contains this point?", a pure `Sphere::contains` (altitude vs the layer boundary): robust exactly on the rock surface and structurally unable to return the rock. Since the atmosphere is a single-child nested-ball chain and the rock mesh is only ever a non-index-0 child of its owning layer, descending the index-0 child chain while each layer's volume contains the point yields the innermost containing layer (inner layer below its boundary, next layer above) without ever reaching the rock. Ownership therefore only governs the `getParent()`-never-universe safety; exit-layer placement is exact and ping-pong-free.

**Nudging the particle clear — a perpendicular clearance off the exit face.** Having chosen the layer, the relocator displaces the particle along its own trajectory — the exit direction at a rock-exit crossing — but scaled so the *perpendicular* separation from the exit triangle's plane is 2 µm (twice the mesh `boundaryPadding`). The along-track distance applied is `2 µm / |dir·n|`, where `n` is the exit triangle's normal taken from the ray–mesh hit (`MeshRayHit::normal` of the nearest `intersectRayAll` crossing). This correction is designed to avoid a few tricky edge cases:
- Scaling by the inverse incidence cosine makes the clearance *perpendicular*-exact independently of how obliquely the trajectory crosses the face.
- The perpendicular nudge guarantee is important because the tracker leaves a rock-exiting particle numerically pinned onto the exit triangle: the step endpoint is computed as `start + velocity·t` in the ECEF frame at radius ~6.4×10⁶ m, so the floating-point residual off the face is O(ε_mach·10⁶ m) ≈ 1 nm — far below `boundaryPadding`. Without a positive perpendicular clearance the next step's intersection test discards that sub-padding self-hit and the next "real" exit collapses to the far terrain face, unreachable in interaction-limited steps; with the clearance applied the next step starts from an unambiguously-in-air point and resolves cleanly in the resolved layer.
- The `|dir·n|` *absolute value* makes this independent of the triangle's winding-defined normal sign, which is necessary because CORSIKA does not canonicalise mesh normals outward
- A near-tangent exit (`|dir·n|→0`, scale diverging) is capped at a 1 mm along-track displacement (with a warning), and if the volume is not a mesh or returns no hits it falls back to a plain 2 µm along-trajectory step. 

**Why both corrections take effect immediately.** `Cascade::step` calls boundary-crossing processes immediately after assigning the tracker's node and before the next step (then returns) — and the next step's `getTrack` reads `getPosition()` fresh, with no pre-computed trajectory — so both the node correction and the position nudge are in force on the very next step. If the particle is still skimming the terrain it re-enters rock normally on a later step via the excluded-overlap candidate set.
...
- `detail/framework/geometry/TriangularMesh.inl` — `contains()` (lines 63–95) is a 3-ray parity vote that excludes hits within `boundaryPadding` (1 µm, line 88); this is exactly why a point left *on* the rock surface is degenerate under `getContainingNode`, and why `RockExitRelocator` resolves among layer spheres instead. `node.getVolume().contains(p)` (the layer-sphere test it reuses) is the same primitive `Intersect.inl:29` and `getContainingNode` use. `intersectRayAll` returns `MeshRayHit`s sorted by distance, each carrying the intersected triangle's `normal` (`framework/geometry/TriangularMesh.hpp`); `RockExitRelocator` reads the nearest hit's `normal` to size its perpendicular-clearance nudge. The normal is the triangle's winding normal (`(edge1×edge2).normalized()`, `Triangle.inl`), not canonicalised outward — which is why the relocator uses `|dir·n|` (sign-independent).
- `tambo_shower.cpp` (local, [src/corsika/tambo_shower/src/](../src/corsika/tambo_shower/src/)) — `RockExitRelocator` re-resolves the logical node by layer-only sphere descent and nudges the particle a defined perpendicular clearance off the exit face on every rock exit (along the trajectory, rescaled by `1/|dir·n|` from the `MeshRayHit::normal`); `RockInterfaceTripwire` (a passive `ContinuousProcess`) aborts the run if the logical-node-implies-inside invariant is persistently violated.
...
The volume tree at the root is `Universe`, an infinite-radius `Sphere` whose `contains(p)` always returns true. Universe is *not* given any `HomogeneousMedium` or other model properties in `tambo_shower` — it is an empty container. Its direct child is the largest atmosphere layer (the 5000 km-radius outermost ball, in the shipped `colca.toml` site file). Since the profile now comes from a site file, `tambo_shower` refuses to start if the *injection point* lies at or above that outermost boundary — otherwise the primary itself would be erased here before the cascade began. Secondaries that later step outward past the boundary are still erased as described below; that is expected. See `resources/sites/README.md`. When a particle inside that outermost layer steps far enough outward to cross its surface, the tracking machinery in `Intersect.inl` resolves the next volume as `volumeNode.getParent()` — which is Universe. At that point `Cascade::step` (lines 278–281 of `Cascade.inl`) sees `nextVol == environment_.getUniverse().get()`, calls `particle.erase()` *before* updating the particle's logical node, and emits a log message reading:
