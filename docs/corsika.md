...
**Nudging the particle clear — a perpendicular clearance off the exit face.** Having chosen the layer, the relocator displaces the particle along its own trajectory — the exit direction at a rock-exit crossing — but scaled so the *perpendicular* separation from the exit triangle's plane is 2 µm (twice the mesh `boundaryPadding`). The along-track distance applied is `2 µm / |dir·n|`, where `n` is the exit triangle's normal taken from the ray–mesh hit (`MeshRayHit::normal` of the nearest `intersectRayAll` crossing). Scaling by the inverse incidence cosine makes the clearance *perpendicular*-exact independently of how obliquely the trajectory crosses the face: a fixed along-track step would clear only `2 µm·|dir·n|` perpendicular, which an oblique exit can drive below `boundaryPadding` and back into the padding-degenerate band. The `|dir·n|` *absolute value* makes this independent of the triangle's winding-defined normal sign — CORSIKA does not canonicalise mesh normals outward, so `n`'s sign is unreliable, but advancing along the trajectory by a positive distance is the correct exit move regardless. A near-tangent exit (`|dir·n|→0`, scale diverging) is capped at a 1 mm along-track displacement (with a warning), and if the volume is not a mesh or returns no hits it falls back to a plain 2 µm along-trajectory step. This perpendicular guarantee matters because the tracker leaves a rock-exiting particle numerically pinned onto the exit triangle: the step endpoint is computed as `start + velocity·t` in the ECEF frame at radius ~6.4×10⁶ m, so the floating-point residual off the face is O(ε_mach·10⁶ m) ≈ 1 nm — far below `boundaryPadding`. Without a positive perpendicular clearance the next step's intersection test discards that sub-padding self-hit and the next "real" exit collapses to the far terrain face, unreachable in interaction-limited steps; with the clearance applied the next step starts from an unambiguously-in-air point and resolves cleanly in the resolved layer.
