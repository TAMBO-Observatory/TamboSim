...
Parse command-line arguments for the proton CORSIKA shower script.
...
function main()
    ...
    seed = abs(hash(base_pinecone + simset_id * 1000000 + event_id * 1000)) % typemax(Int32)

    # Create output directory (one shower per proton, so shower_1)
