...
// ---------------------------------------------------------------------------
// Direction filter for the one-way absorbing box faces.
//
// OutwardFilter decides which particles a given box face acts on: it returns
// true when the particle's direction has a positive component along the
// face's outward normal (i.e. the particle is leaving the box) and false
// otherwise.  Used with make_select so that each face only absorbs outgoing
// particles and is invisible to inbound ones.
//
...
// has_getDirection is a compile-time check that lets OutwardFilter safely
// return false for those objects instead of crashing.
//
// OutwardFilter is defined here at file scope rather than inside main() due
// to a compiler limitation in GCC 8: member function templates inside
// locally-defined structs are not handled correctly, causing spurious compile
// errors.
// ---------------------------------------------------------------------------
...
struct OutwardFilter {
  std::array<double, 3> outHat_;
  ...
  bool operator()(TParticle const& p) const {
    if constexpr (has_getDirection<TParticle>::value) {
      ...
      return ev.dot(Eigen::Vector3d{outHat_[0], outHat_[1], outHat_[2]}) > 0.0;
    } else {
...
int main(int argc, char** argv) {
  ...
  app.add_option("--intercept-z",
      ...
      ->required()
      ->group("Geometry");
  app.add_option("--box-padding-km",
                 "If set, build a one-way absorbing box this many km outside the "
                 "obs-mesh extremum on each of its five faces (top, east, west, "
                 "north, south). If omitted, the box is not built.")
      ->check(CLI::PositiveNumber)
      ->group("Geometry");
  ...
  bool   const useBox      = app.count("--box-padding-km") > 0;
  double const boxPaddingM = useBox ? app["--box-padding-km"]->as<double>() * 1e3 : 0.0;
  ...
  // Mean-of-vertices centroid of the obs mesh in ECEF metres.  Used as the
  // origin of the one-way absorbing box that bounds shower propagation.
  double cxObs = 0.0, cyObs = 0.0, czObs = 0.0;
  size_t const nVertObs = obsMesh.getVertexCount();
  for (size_t vi = 0; vi < nVertObs; ++vi) {
    auto const coords = obsMesh.getVertex(vi).getCoordinates(rootCS);
    cxObs += coords.getX() / 1_m;
    cyObs += coords.getY() / 1_m;
    czObs += coords.getZ() / 1_m;
  }
  cxObs /= double(nVertObs);
  cyObs /= double(nVertObs);
  czObs /= double(nVertObs);
  CORSIKA_LOG_INFO("Obs-mesh centroid (ECEF m): ({:.1f}, {:.1f}, {:.1f})",
                   cxObs, cyObs, czObs);
  ...
  /* === ONE-WAY ABSORBING BOX hugging the obs mesh (optional) ===
   *
   * Enabled iff --box-padding-km is set.  Five open faces (top, east, west,
   * north, south; bottom open) bound the volume in which the shower is
   * tracked.  Each face sits boxPaddingM metres outside the corresponding
   * extremum of the obs mesh along that face's outward normal:
   *   top   = max_v (v - centroid)·upHat       + pad
   *   east  = max_v (v - centroid)·eastHat     + pad
   *   west  = max_v (v - centroid)·(-eastHat)  + pad
   *   north = max_v (v - centroid)·northHat    + pad
   *   south = max_v (v - centroid)·(-northHat) + pad
   * Each face is one-way absorbing: particles travelling outward (velocity
   * component along the face's outward normal > 0) are removed; inbound
   * particles pass through unaffected (NullModel branch in the make_select
   * wiring below).  Hits on these faces are discarded -- the box is purely
   * a compute-saver.
   */
  std::array<double, 3> outTop{}, outEast{}, outWest{}, outNorth{}, outSouth{};
  std::optional<Plane> topPlane, eastPlane, westPlane, northPlane, southPlane;

  if (useBox) {
    std::array<double, 3> eastHatBox, northHatBox, upHatBox;
    ecefToENU(cxObs, cyObs, czObs, eastHatBox, northHatBox, upHatBox);

    outTop   = upHatBox;
    outEast  = eastHatBox;
    outWest  = {-eastHatBox[0],  -eastHatBox[1],  -eastHatBox[2]};
    outNorth = northHatBox;
    outSouth = {-northHatBox[0], -northHatBox[1], -northHatBox[2]};

    // Maximum projection of (vertex - centroid) onto an outward direction.
    auto const maxProjAlong = [&](std::array<double, 3> const& outHat) {
      double m = -std::numeric_limits<double>::infinity();
      for (size_t vi = 0; vi < nVertObs; ++vi) {
        auto const c = obsMesh.getVertex(vi).getCoordinates(rootCS);
        double const dx = c.getX() / 1_m - cxObs;
        double const dy = c.getY() / 1_m - cyObs;
        double const dz = c.getZ() / 1_m - czObs;
        m = std::max(m, dx * outHat[0] + dy * outHat[1] + dz * outHat[2]);
      }
      return m;
    };

    double const topExtentM   = maxProjAlong(outTop)   + boxPaddingM;
    double const eastExtentM  = maxProjAlong(outEast)  + boxPaddingM;
    double const westExtentM  = maxProjAlong(outWest)  + boxPaddingM;
    double const northExtentM = maxProjAlong(outNorth) + boxPaddingM;
    double const southExtentM = maxProjAlong(outSouth) + boxPaddingM;

    auto const makeFaceCenter = [&](std::array<double, 3> const& outHat,
                                    double extentM) {
      return Point{rootCS,
          (cxObs + outHat[0] * extentM) * 1_m,
          (cyObs + outHat[1] * extentM) * 1_m,
          (czObs + outHat[2] * extentM) * 1_m};
    };

    Point const topCenter   = makeFaceCenter(outTop,   topExtentM);
    Point const eastCenter  = makeFaceCenter(outEast,  eastExtentM);
    Point const westCenter  = makeFaceCenter(outWest,  westExtentM);
    Point const northCenter = makeFaceCenter(outNorth, northExtentM);
    Point const southCenter = makeFaceCenter(outSouth, southExtentM);

    // Plane normals point inward; absorption direction is set by OutwardFilter.
    DirectionVector const topNormal  {rootCS, {-outTop[0],   -outTop[1],   -outTop[2]}};
    DirectionVector const eastNormal {rootCS, {-outEast[0],  -outEast[1],  -outEast[2]}};
    DirectionVector const westNormal {rootCS, {-outWest[0],  -outWest[1],  -outWest[2]}};
    DirectionVector const northNormal{rootCS, {-outNorth[0], -outNorth[1], -outNorth[2]}};
    DirectionVector const southNormal{rootCS, {-outSouth[0], -outSouth[1], -outSouth[2]}};

    topPlane.emplace  (topCenter,   topNormal);
    eastPlane.emplace (eastCenter,  eastNormal);
    westPlane.emplace (westCenter,  westNormal);
    northPlane.emplace(northCenter, northNormal);
    southPlane.emplace(southCenter, southNormal);

    CORSIKA_LOG_INFO("Box padding: {:.3f} km", boxPaddingM / 1e3);
    CORSIKA_LOG_INFO("Box top   face: {:.1f} m from obs-mesh centroid", topExtentM);
    CORSIKA_LOG_INFO("Box east  face: {:.1f} m from obs-mesh centroid", eastExtentM);
    CORSIKA_LOG_INFO("Box west  face: {:.1f} m from obs-mesh centroid", westExtentM);
    CORSIKA_LOG_INFO("Box north face: {:.1f} m from obs-mesh centroid", northExtentM);
    CORSIKA_LOG_INFO("Box south face: {:.1f} m from obs-mesh centroid", southExtentM);
  } else {
    CORSIKA_LOG_INFO("Absorbing box disabled (no --box-padding-km)");
  }
  ...
  /* === ONE-WAY ABSORBING BOX FACES: top, east, west, north, south ===
   * Each face absorbs particles whose direction has a positive component
   * along that face's outward normal (i.e. particles leaving the box).
   * Inbound particles take the NullModel branch and pass through.  The
   * face writers are registered with OutputManager but their data is
   * discarded -- the box only saves compute by terminating tracks
   * that can no longer reach the obs mesh.  Skipped entirely when
   * --box-padding-km was not provided.
   */
  std::optional<ObservationPlane<TrackingType, ParticleWriterParquet>>
      topLevel, eastLevel, westLevel, northLevel, southLevel;
  if (useBox) {
    topLevel.emplace  (*topPlane,   escapeRefDir, true, 1e-6_m);
    eastLevel.emplace (*eastPlane,  escapeRefDir, true, 1e-6_m);
    westLevel.emplace (*westPlane,  escapeRefDir, true, 1e-6_m);
    northLevel.emplace(*northPlane, escapeRefDir, true, 1e-6_m);
    southLevel.emplace(*southPlane, escapeRefDir, true, 1e-6_m);
    output.add("box_face_top",   *topLevel);
    output.add("box_face_east",  *eastLevel);
    output.add("box_face_west",  *westLevel);
    output.add("box_face_north", *northLevel);
    output.add("box_face_south", *southLevel);
  }
  ...
  for (int i = 1; i <= nev; ++i) {
    ...
    if (useBox) {
      auto faceTop   = make_select(OutwardFilter{outTop},   *topLevel,   NullModel{});
      auto faceEast  = make_select(OutwardFilter{outEast},  *eastLevel,  NullModel{});
      auto faceWest  = make_select(OutwardFilter{outWest},  *westLevel,  NullModel{});
      auto faceNorth = make_select(OutwardFilter{outNorth}, *northLevel, NullModel{});
      auto faceSouth = make_select(OutwardFilter{outSouth}, *southLevel, NullModel{});
      if (useTerrainMesh) {
        auto obsMeshSequence = make_sequence(observationLevel, *terrainLevel,
                                             faceTop, faceEast, faceWest, faceNorth, faceSouth,
                                             escapeLevel);
