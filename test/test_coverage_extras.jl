...
function run_display_tests()
    ...
    @testset "Display methods" begin
        ...
        _qp = Dict{Char,Frame}('G' => _gf, 'M' => _mf)
        ...
        g_frame = Frame('G', Dict{String,Any}("parent_key" => 42))
        m_frame_inner = Frame('M', Dict{String,Any}())
        child_f = Frame('Q', Dict{String,Any}("child_key" => 7), Dict{Char,Frame}('G' => g_frame, 'M' => m_frame_inner))
        show(io, child_f)
